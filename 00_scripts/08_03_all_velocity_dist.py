...
for startHght in altInds:
    ...
    binMax = binMax + 0.5
    #binMax = binMax + 1 if binMax % 2 == 0 else binMax
    binMin = np.floor(an.vars['zW'].min)
    binMin = binMin - 0.5
    #binMin = binMin - 1 if binMin % 2 == 0 else binMin
    #print(binMin)
    #print(binMax)
    ...
    #plt.ylim((1E-8,1E0))
    #plt.title('Vertical Velocity Distribution at altitude '+str(altitude))
    #plt.grid()
    #if i_plot == 2:
    #    outName = 'vertical_velocity_dist_alt_'+str(altitude)+'.png'
    ...
    #    ax.grid()
    #    ax.set_title(str(an.resolutions[i]))
    #plt.suptitle('Vertical Velocity Distribution at altitude '+str(altitude))
    #plt.subplots_adjust(left=0.07, right=0.95, top=0.92, bottom=0.08, wspace=0.27, hspace=0.33)
    #if i_plot == 2:
    #    outName = 'vertical_velocity_dist_and_diff_alt_'+str(altitude)+'.png'
    #    plt.savefig(outPath + outName)
    #elif i_plot == 1:
    #    plt.show()
    #plt.close(fig)
