...
! `/opt/cray/netcdf/4.3.3.1/bin/nf-config --fflags --flibs`                           !
!                                                                                   !
...
!     enddo
  ...
  nxny2 = sqrt(real(nx)**2+real(ny)**2)
  ...
  do t=1,nt
     ...
     if (maxt.gt.0) then
        ...
        do m = 1,maxt ! loop over clusters
           ...
           cmf(m,t)   = sum(mf(:,:,t),maskl3)/max(1,count(maskl3)) ! lowest cloud base found in the cloud
           if (count(maskl3)==0) cmf(m,t) = -999. ! set to missing value if cloud has no base
           cwm(m,t)   = sum(wm(:,:,t),maskl2)/max(1,count(maskl2)) ! highest cloud top found in the cloud
           dqvsm(m,t) = sum(dqvsdz(:,:,t),maskl3)/max(1,count(maskl3)) ! mean vertical qvs gradient
           if (count(maskl3)==0) dqvsm(m,t) = -999. ! set to missing value if cloud has no base
           qvsm(m,t)  = sum(qvs_cb(:,:,t),maskl2)/max(1,count(maskl2)) ! mean saturation specific humidity
           if (count(maskl2)==0) qvsm(m,t)  = -999. ! set to missing value if cloud has no base
           ...
           ad(m,t)   = sum(lwp_in(:,:,t)/lwp_ad_in(:,:,t),maskl2)/max(1,count(maskl2)) ! mean ratio of lwp to lwp_ad
        ...
        do m = 1,maxt ! loop over clusters
           ...
           do n = 1,maxt ! loop over the other clusters
              if (n.ne.m) then
                 ! the domain is periodic, thus we need to check for the "direct" distance and the one 
                 ! across the borders
                 dist1 = sqrt((xcenter(m,t)-xcenter(n,t))**2+(ycenter(m,t)-ycenter(n,t))**2)
                 dist2 = sqrt((xcenter(m,t)-xcenter(n,t)+real(nx)-6.)**2+(ycenter(m,t)-ycenter(n,t))**2)
                 dist3 = sqrt((xcenter(m,t)-xcenter(n,t))**2+(ycenter(m,t)-ycenter(n,t)+real(ny)-6.)**2)
                 dist4 = sqrt((xcenter(m,t)-xcenter(n,t)+real(nx)-6.)**2+(ycenter(m,t)-ycenter(n,t)+real(ny)-6.)**2)
